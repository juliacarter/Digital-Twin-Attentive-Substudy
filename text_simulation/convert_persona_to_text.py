...
def _format_question_text_Matrix(question: dict, with_answers: bool = False, is_attentive: bool = False, attentive_reverse=False, attentive_scale=-1) -> str:
    ...
    if columns:
        ...
        if is_attentive & attentive_reverse:
...
def _format_question_text_MC(question: dict, with_answers: bool = False, is_attentive: bool = False, attentive_reverse=False, attentive_scale=-1) -> str:
    ...
    if options:
        ...
        if is_attentive & attentive_reverse:
            ...
            pass
        if is_attentive & (attentive_scale != -1):
            extra = attentive_scale % (len(options)-1)
            each = int(attentive_scale / (len(options)-1))
            new_columns = []
            answer_positions = []
            for i, column_text in enumerate(options, 1):
                answer_positions.append(len(options))
                new_columns.append(column_text)
                if i != len(columns):
                    for i in range(each):
                        new_columns.append("")
...
def _format_question_text_Slider(question: dict, with_answers: bool = False, is_attentive: bool = False, attentive_reverse: bool = False, attentive_scale = 100) -> str:
    ...
    if values:
        ...
        for i, value_item in enumerate(values):
            ...
            if is_attentive & (attentive_scale != -1):
                old_max = question.get("Range", {}).get("Max", 100)
                new_scale = attentive_scale / old_max
                for i, answer in enumerate(answers):
                    answers[i] = answer * new_scale
...
def format_question_text(question: dict, with_answers: bool = False, attentive_id="-1", attentive_reverse=False, attentive_scale=-1) -> str:
    """Formats a single question text into a readable string."""
    ...
    if question_type == "Matrix":
        output_options = _format_question_text_Matrix(question, with_answers, is_attentive, attentive_reverse, attentive_scale)
    elif question_type == "MC": # Multiple Choice
        output_options = _format_question_text_MC(question, with_answers, is_attentive, attentive_reverse, attentive_scale)
    elif question_type == "TE": # Text Entry
        output_options = _format_question_text_TE(question, with_answers, is_attentive, attentive_reverse, attentive_scale)
    elif question_type == "Slider":
        output_options = _format_question_text_Slider(question, with_answers, is_attentive, attentive_reverse, attentive_scale)
